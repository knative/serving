...
func init() {
	injection.Default.RegisterInformerFactory(withCachingInformerFactory)
}
...
func withCachingInformerFactory(ctx context.Context) context.Context {
	sc := cachingclient.Get(ctx)
	return context.WithValue(ctx, Key{},
		informers.NewSharedInformerFactory(sc, controller.GetResyncPeriod(ctx)))
}

// Get extracts the Caching InformerFactory from the context.

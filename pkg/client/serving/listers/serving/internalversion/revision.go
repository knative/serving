...
type RevisionLister interface {
	...
	List(selector labels.Selector) (ret []*serving.Revision, err error)
...
// List lists all Revisions in the indexer.
func (s *revisionLister) List(selector labels.Selector) (ret []*serving.Revision, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*serving.Revision))
	})
	return ret, err
}
...
type RevisionNamespaceLister interface {
	...
	Get(name string) (*serving.Revision, error)
...
// List lists all Revisions in the indexer for a given namespace.
func (s revisionNamespaceLister) List(selector labels.Selector) (ret []*serving.Revision, err error) {
...
// Get retrieves the Revision from the indexer for a given namespace and name.
func (s revisionNamespaceLister) Get(name string) (*serving.Revision, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(serving.Resource("revision"), name)
	}
	return obj.(*serving.Revision), nil
}

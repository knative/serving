...
type ConfigurationLister interface {
	...
	List(selector labels.Selector) (ret []*serving.Configuration, err error)
...
// List lists all Configurations in the indexer.
func (s *configurationLister) List(selector labels.Selector) (ret []*serving.Configuration, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*serving.Configuration))
	})
	return ret, err
}
...
type ConfigurationNamespaceLister interface {
	...
	Get(name string) (*serving.Configuration, error)
...
// List lists all Configurations in the indexer for a given namespace.
func (s configurationNamespaceLister) List(selector labels.Selector) (ret []*serving.Configuration, err error) {
...
// Get retrieves the Configuration from the indexer for a given namespace and name.
func (s configurationNamespaceLister) Get(name string) (*serving.Configuration, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(serving.Resource("configuration"), name)
	}
	return obj.(*serving.Configuration), nil
}

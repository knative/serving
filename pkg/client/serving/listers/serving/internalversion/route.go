...
type RouteLister interface {
	...
	List(selector labels.Selector) (ret []*serving.Route, err error)
...
// List lists all Routes in the indexer.
func (s *routeLister) List(selector labels.Selector) (ret []*serving.Route, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*serving.Route))
	})
	return ret, err
}
...
type RouteNamespaceLister interface {
	...
	Get(name string) (*serving.Route, error)
...
// List lists all Routes in the indexer for a given namespace.
func (s routeNamespaceLister) List(selector labels.Selector) (ret []*serving.Route, err error) {
...
// Get retrieves the Route from the indexer for a given namespace and name.
func (s routeNamespaceLister) Get(name string) (*serving.Route, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(serving.Resource("route"), name)
	}
	return obj.(*serving.Route), nil
}

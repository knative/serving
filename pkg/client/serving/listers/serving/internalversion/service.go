...
type ServiceLister interface {
	...
	List(selector labels.Selector) (ret []*serving.Service, err error)
...
// List lists all Services in the indexer.
func (s *serviceLister) List(selector labels.Selector) (ret []*serving.Service, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*serving.Service))
	})
	return ret, err
}
...
type ServiceNamespaceLister interface {
	...
	Get(name string) (*serving.Service, error)
...
// List lists all Services in the indexer for a given namespace.
func (s serviceNamespaceLister) List(selector labels.Selector) (ret []*serving.Service, err error) {
...
// Get retrieves the Service from the indexer for a given namespace and name.
func (s serviceNamespaceLister) Get(name string) (*serving.Service, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(serving.Resource("service"), name)
	}
	return obj.(*serving.Service), nil
}

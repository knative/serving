...
type DomainMappingLister interface {
	...
	List(selector labels.Selector) (ret []*v1.DomainMapping, err error)
...
// List lists all DomainMappings in the indexer.
func (s *domainMappingLister) List(selector labels.Selector) (ret []*v1.DomainMapping, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.DomainMapping))
	})
	return ret, err
}
...
type DomainMappingNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.DomainMapping, error)
...
// List lists all DomainMappings in the indexer for a given namespace.
func (s domainMappingNamespaceLister) List(selector labels.Selector) (ret []*v1.DomainMapping, err error) {
...
// Get retrieves the DomainMapping from the indexer for a given namespace and name.
func (s domainMappingNamespaceLister) Get(name string) (*v1.DomainMapping, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("domainmapping"), name)
	}
	return obj.(*v1.DomainMapping), nil
}

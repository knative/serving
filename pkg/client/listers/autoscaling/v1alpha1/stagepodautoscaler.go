...
type StagePodAutoscalerLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.StagePodAutoscaler, err error)
...
// List lists all StagePodAutoscalers in the indexer.
func (s *stagePodAutoscalerLister) List(selector labels.Selector) (ret []*v1alpha1.StagePodAutoscaler, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.StagePodAutoscaler))
	})
	return ret, err
}
...
type StagePodAutoscalerNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.StagePodAutoscaler, error)
...
// List lists all StagePodAutoscalers in the indexer for a given namespace.
func (s stagePodAutoscalerNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.StagePodAutoscaler, err error) {
...
// Get retrieves the StagePodAutoscaler from the indexer for a given namespace and name.
func (s stagePodAutoscalerNamespaceLister) Get(name string) (*v1alpha1.StagePodAutoscaler, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("stagepodautoscaler"), name)
	}
	return obj.(*v1alpha1.StagePodAutoscaler), nil
}

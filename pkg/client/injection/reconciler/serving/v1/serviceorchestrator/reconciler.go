...
// controller reconciling v1.ServiceOrchestrator.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.ServiceOrchestrator. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1.ServiceOrchestrator) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.ServiceOrchestrator.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.ServiceOrchestrator. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.ServiceOrchestrator) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.ServiceOrchestrator if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.ServiceOrchestrator.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.ServiceOrchestrator) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.ServiceOrchestrator) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.ServiceOrchestrator resources.
type reconcilerImpl struct {
	...
	Lister servingv1.ServiceOrchestratorLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister servingv1.ServiceOrchestratorLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.ServiceOrchestrators(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1.ServiceOrchestrator, desired *v1.ServiceOrchestrator) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.ServingV1().ServiceOrchestrators(desired.Namespace)
		...
		updater := r.Client.ServingV1().ServiceOrchestrators(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.ServiceOrchestrator, desiredFinalizers sets.String) (*v1.ServiceOrchestrator, error) {
	// Don't modify the informers copy.
	...
	patcher := r.Client.ServingV1().ServiceOrchestrators(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.ServiceOrchestrator) (*v1.ServiceOrchestrator, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.ServiceOrchestrator, reconcileEvent reconciler.Event) (*v1.ServiceOrchestrator, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {

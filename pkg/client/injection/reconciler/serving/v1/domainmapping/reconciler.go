...
import (
	...
	servingv1types "knative.dev/serving/pkg/apis/serving/v1"
...
// controller reconciling v1.DomainMapping.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.DomainMapping. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *servingv1types.DomainMapping) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.DomainMapping.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.DomainMapping. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *servingv1types.DomainMapping) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.DomainMapping if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.DomainMapping.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *servingv1types.DomainMapping) reconciler.Event
}

type doReconcile func(ctx context.Context, o *servingv1types.DomainMapping) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.DomainMapping resources.
type reconcilerImpl struct {
	...
	Lister servingv1.DomainMappingLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister servingv1.DomainMappingLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *servingv1types.DomainMapping, desired *servingv1types.DomainMapping) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.ServingV1().DomainMappings(desired.Namespace)
		...
		updater := r.Client.ServingV1().DomainMappings(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *servingv1types.DomainMapping, desiredFinalizers sets.Set[string]) (*servingv1types.DomainMapping, error) {
...
func (r *reconcilerImpl) updateFinalizersFilteredServerSideApply(ctx context.Context, resource *servingv1types.DomainMapping, desiredFinalizers sets.Set[string]) (*servingv1types.DomainMapping, error) {
	...
	patcher := r.Client.ServingV1().DomainMappings(resource.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFilteredMergePatch(ctx context.Context, resource *servingv1types.DomainMapping, desiredFinalizers sets.Set[string]) (*servingv1types.DomainMapping, error) {
	// Don't modify the informers copy.
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *servingv1types.DomainMapping) (*servingv1types.DomainMapping, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *servingv1types.DomainMapping, reconcileEvent reconciler.Event) (*servingv1types.DomainMapping, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
	...
	if err != nil {
		...
		getter := r.Client.ServingV1().DomainMappings(resource.Namespace)

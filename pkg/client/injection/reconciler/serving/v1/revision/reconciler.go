...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.Revision) (*v1.Revision, error) {
	...
	resource, err = r.Client.ServingV1().Revisions(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.Revision, reconcileEvent reconciler.Event) (reconciler.Event, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {

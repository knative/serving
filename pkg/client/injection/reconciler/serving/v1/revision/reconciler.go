...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	original, err := r.Lister.Revisions(namespace).Get(name)
...
func (r *reconcilerImpl) updateStatus(existing *v1.Revision, desired *v1.Revision) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {
			existing, err = r.Client.ServingV1().Revisions(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.ServingV1().Revisions(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.Revision) error {
	finalizerName := defaultFinalizerName

	actual, err := r.Lister.Revisions(resource.Namespace).Get(resource.Name)
	if err != nil {
		return err
	}
	...
	_, err = r.Client.ServingV1().Revisions(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.Revision) error {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.Revision, reconcileEvent reconciler.Event) error {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return nil
	}
	if resource.GetDeletionTimestamp().IsZero() {

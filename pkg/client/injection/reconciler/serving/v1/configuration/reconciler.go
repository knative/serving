...
func (r *reconcilerImpl) updateStatus(existing *v1.Configuration, desired *v1.Configuration) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {
			existing, err = r.Client.ServingV1().Configurations(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.ServingV1().Configurations(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.Configuration) (*v1.Configuration, error) {
	...
	resource, err = r.Client.ServingV1().Configurations(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {

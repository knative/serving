...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.PodAutoscaler, desired *v1alpha1.PodAutoscaler) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {
			existing, err = r.Client.AutoscalingV1alpha1().PodAutoscalers(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.AutoscalingV1alpha1().PodAutoscalers(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.PodAutoscaler, reconcileEvent reconciler.Event) (reconciler.Event, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {

...
// controller reconciling v1alpha1.Ingress if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.Ingress.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.Ingress) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.Ingress if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1alpha1.Ingress.
	// This method should not write to the API.
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.Ingress, desired *v1alpha1.Ingress) error {
	...
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		...
		if attempts > 0 {
			...
			getter := r.Client.NetworkingV1alpha1().Ingresses(desired.Namespace)
		...
		updater := r.Client.NetworkingV1alpha1().Ingresses(existing.Namespace)
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.Ingress, reconcileEvent reconciler.Event) (*v1alpha1.Ingress, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {

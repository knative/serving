...
// controller reconciling v1alpha1.Ingress.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.Ingress. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.Ingress) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.Ingress.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.Ingress. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.Ingress) reconciler.Event
}
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister networkingv1alpha1.IngressLister, recorder record.EventRecorder, r Interface) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.Ingress, desired *v1alpha1.Ingress) error {
	...
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		...
		if attempts > 0 {
			existing, err = r.Client.NetworkingV1alpha1().Ingresses(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.NetworkingV1alpha1().Ingresses(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.Ingress) error {
	finalizerName := defaultFinalizerName

	actual, err := r.Lister.Ingresses(resource.Namespace).Get(resource.Name)
	if err != nil {
		return err
	}
	...
	_, err = r.Client.NetworkingV1alpha1().Ingresses(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.Ingress) error {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.Ingress, reconcileEvent reconciler.Event) error {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return nil
	}
	if resource.GetDeletionTimestamp().IsZero() {

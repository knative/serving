...
func Get(ctx context.Context, selector string) v1.StagePodAutoscalerInformer {
	untyped := ctx.Value(Key{Selector: selector})
	if untyped == nil {
		logging.FromContext(ctx).Panicf(
			"Unable to fetch knative.dev/serving/pkg/client/informers/externalversions/serving/v1.StagePodAutoscalerInformer with selector %s from context.", selector)
	}

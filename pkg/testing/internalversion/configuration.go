...
type ConfigOption func(*serving.Configuration)

// WithConfigDeletionTimestamp will set the DeletionTimestamp on the Config.
func WithConfigDeletionTimestamp(r *serving.Configuration) {
	t := metav1.NewTime(time.Unix(1e9, 0))
	r.ObjectMeta.SetDeletionTimestamp(&t)
}

// WithConfigOwnersRemoved clears the owner references of this Configuration.
func WithConfigOwnersRemoved(cfg *serving.Configuration) {
	cfg.OwnerReferences = nil
}

// WithConfigContainerConcurrency sets the given Configuration's concurrency.
func WithConfigContainerConcurrency(cc serving.RevisionContainerConcurrencyType) ConfigOption {
	return func(cfg *serving.Configuration) {
...
func WithObservedGen(cfg *serving.Configuration) {
...
func WithConfigLabel(key, value string) ConfigOption {
	return func(config *serving.Configuration) {

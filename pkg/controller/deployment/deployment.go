...
type Receiver interface {
	SyncDeployment(*appsv1.Deployment) error
}

// Controller implements the controller for Deployment resources
...
func NewController(
	...
	// obtain references to a shared index informer for the Deployment type.
	...
	controller := &Controller{
		Base: controller.NewBase(kubeClientSet, elaClientSet, kubeInformerFactory,
			elaInformerFactory, informer.Informer(), controllerAgentName, "Deployments", logger),
	...
	if len(controller.receivers) == 0 {
		return nil, fmt.Errorf("None of the provided receivers implement deployment.Receiver. " +
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	return c.RunController(threadiness, stopCh,
		[]cache.InformerSynced{c.synced}, c.syncHandler, "Deployment")
}

// syncHandler compares the actual state with the desired, and attempts to
// converge the two. It then updates the Status block of the Deployment
...
func (c *Controller) syncHandler(key string) error {
	...
	original, err := c.lister.Deployments(namespace).Get(name)
	if err != nil {
		// The resource may no longer exist, in which case we stop
		// processing.
		if errors.IsNotFound(err) {
			runtime.HandleError(fmt.Errorf("deployment %q in work queue no longer exists", key))
			return nil
		}
	...
	for _, dr := range c.receivers {
		...
		if err := dr.SyncDeployment(cp); err != nil {
			return err
		}
	}
	return nil

...
type Receiver interface {
	SyncRoute(*v1alpha1.Route) error
}
...
// NewController creates a new Route controller
func NewController(
	...
	// obtain references to a shared index informer for the Route type.
	...
	if len(controller.receivers) == 0 {
		return nil, fmt.Errorf("None of the provided receivers implement route.Receiver. " +
			"If the Route controller is no longer needed it should be removed.")
	}
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	return c.RunController(threadiness, stopCh,
		[]cache.InformerSynced{c.synced}, c.syncHandler, "Route")
}

// syncHandler compares the actual state with the desired, and attempts to
// converge the two. It then updates the Status block of the Route
...
func (c *Controller) syncHandler(key string) error {
	...
	original, err := c.lister.Routes(namespace).Get(name)
	if err != nil {
	...
	for _, dr := range c.receivers {
		...
		if err := dr.SyncRoute(cp); err != nil {
			return err
		}
	}
	return nil

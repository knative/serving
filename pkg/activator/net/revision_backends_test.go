...
func TestRevisionWatcher(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		meshMode: config.MeshCompatibilityModeEnabled,
		probeHostResponses: map[string][]activatortest.FakeResponse{
			"128.0.0.1:1234": {{
		...
		meshMode: config.MeshCompatibilityModeDisabled,
		probeHostResponses: map[string][]activatortest.FakeResponse{
			"129.0.0.1:1234": {{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.meshMode == "" {
				tc.meshMode = config.MeshCompatibilityModeAuto
			}
...
func TestRevisionBackendManagerAddEndpoint(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			rbm := newRevisionBackendsManagerWithProbeFrequency(ctx, rt, false /*usePassthroughLb*/, config.MeshCompatibilityModeAuto, probeFreq)
			defer func() {
				cancel()
				waitInformers()
...
func TestCheckDestsReadyToNotReady(t *testing.T) {
	...
	rw := &revisionWatcher{
		...
		meshMode:                config.MeshCompatibilityModeAuto,
...
func TestRevisionDeleted(t *testing.T) {
	...
	fakeRT := activatortest.FakeRoundTripper{}
	rbm := newRevisionBackendsManagerWithProbeFrequency(ctx, pkgnetwork.RoundTripperFunc(fakeRT.RT), false /*usePassthroughLb*/, config.MeshCompatibilityModeAuto, probeFreq)
	defer func() {
		cancel()
		waitInformers()
		waitForRevisionBackendManager(t, rbm)

...
func TestProbeHandler(t *testing.T) {
	examples := []struct {
		...
		headers:        http.Header{header.ProbeKey: []string{queue.Name}},
		expectedStatus: http.StatusBadRequest,
		method:         http.MethodPost,
	}, {
		label:          "filter a POST request containing probe header",
		headers:        http.Header{header.ProbeKey: []string{activator.Name}},
		expectedStatus: http.StatusOK,
		method:         http.MethodPost,
	}, {
		...
		headers:        http.Header{header.ProbeKey: []string{"not-empty"}},
		expectedStatus: http.StatusBadRequest,
		method:         http.MethodGet,
	}, {
		...
		headers:        http.Header{header.ProbeKey: []string{""}},
		passed:         true,
		expectedStatus: http.StatusOK,
		method:         http.MethodPost,
	}}
...
func BenchmarkProbeHandler(b *testing.B) {
	tests := []struct {
		...
		headers: http.Header{header.ProbeKey: []string{"some-other-cool-value"}},
	}, {

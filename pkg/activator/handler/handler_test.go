...
func TestActivationHandlerProxyHeader(t *testing.T) {
	...
	case httpReq := <-interceptCh:
		if got := httpReq.Header.Get(header.ProxyKey); got != activator.Name {
			t.Errorf("Header %q = %q, want: %q", header.ProxyKey, got, activator.Name)
		}
...
func TestActivationHandlerPassthroughLb(t *testing.T) {
	...
	case httpReq := <-interceptCh:
		...
		if got, want := httpReq.Header.Get(header.PassthroughLoadbalancingKey), "true"; got != want {

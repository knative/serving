...
2. `ToReserve` -- the Revision is not receiving traffic; it should be placed into a `Reserve` state (deactivation phase one).
3. `Reserve` --  the Revision is not receiving traffic; ingress should be rerouted through the Activator and the Revision Deployments should be scaled to zero (deactivation phase two).
4. `Retired` -- the Revision is not receiving traffic; ingress is has no route to the Revision; all Revision resources should be torn down.
...
Status:  True
Reason:  Idle
Message: The Revision has not received traffic recently.
```
...
Status:             True
Reason:             Reserve
Message:            The Revision has been placed into Reserve state.
...
First the Autoscaler requests transition to Reserve by updating Revision Spec to
the ToReserve state.  The Revision controller marks the Revision Status as Idle.
Then the Route Controller updates Istio RouteRules to point traffic for the
Revision to the Activator.  Once the route has been successfully updated, the
Route controller updates the Revision Spec to the Reserve state.  Then the
Revision controller updates the underlying Deployment Replica count to 0.

*TODO(#1591): As a workaround for lack of Istio RouteRule Status, the Revision
controller waits at least 10 seconds before actually updating the Deployment
Recplica count, which allows for the route rules to fully propagate.*

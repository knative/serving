...
func TestReconcileResponsiveGC(t *testing.T) {
	table := TableTest{{
		...
		Objects: []runtime.Object{
			...
			simpleK8sService(Route("default", "stale-lastpinned", WithConfigTarget("config")), kingressService(kingressEndpoints())),
			kingressEndpoints(),
			kingressService(kingressEndpoints()),
			localEndpoints(
				simplePlaceholderK8sService(Route("default", "stale-lastpinned", WithConfigTarget("config")), ""),
				Route("default", "stale-lastpinned", WithConfigTarget("config")),
				kingressEndpoints(),
			),
		},

		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < 2; i++ {
				t.Run(fmt.Sprint("iteration", i), func(t *testing.T) {
					logger := logtesting.TestLogger(t)
					initContainerStatuses, statuses, _, err := subject.Resolve(logger, fakeRevision, k8schain.Options{ServiceAccountName: "san"}, sets.NewString("skip"), timeout)
					...
					initContainerStatuses, statuses, _, err = subject.Resolve(logger, fakeRevision, k8schain.Options{}, nil, timeout)
...
func TestRateLimitPerItem(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		start := time.Now()
		initResolution, resolution, _, err := subject.Resolve(logger, revision, k8schain.Options{ServiceAccountName: "san"}, sets.NewString("skip"), 0)
		...
		_, _, _, err = subject.Resolve(logger, revision, k8schain.Options{ServiceAccountName: "san"}, sets.NewString("skip"), 0)
		if err == nil {
	...
	t.Run("Does not affect other revisions", func(t *testing.T) {
		start := time.Now()
		_, resolution, _, err := subject.Resolve(logger, rev("another-revision", "img1", "img2"), k8schain.Options{ServiceAccountName: "san"}, sets.NewString("skip"), 0)
	...
	t.Run("Forget clears per-item rate limit", func(t *testing.T) {
		...
		start := time.Now()
		_, resolution, _, err := subject.Resolve(logger, revision, k8schain.Options{ServiceAccountName: "san"}, sets.NewString("skip"), 0)
		if err != nil || resolution != nil {
			t.Fatalf("Expected Resolve to be nil, nil but got %v, %v", resolution, err)
		}

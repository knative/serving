...
import (
	...
	domainmappingreconciler "knative.dev/serving/pkg/client/injection/reconciler/serving/v1/domainmapping"
...
func TestReconcileTLSEnabled(t *testing.T) {
	table := TableTest{{
		...
		WantEvents: []string{
			...
			Eventf(corev1.EventTypeWarning, "InternalError", `notowned: owner: cert.not.owned.ru with Type *v1.DomainMapping does not own Certificate: "cert.not.owned.ru"`),
		},
	}, {
...
type domainMappingOption func(dm *servingv1.DomainMapping)

func domainMapping(namespace, name string, opt ...domainMappingOption) *servingv1.DomainMapping {
...
func withAnnotations(ans map[string]string) domainMappingOption {
	return func(dm *servingv1.DomainMapping) {
		dm.Annotations = ans
	}
}
...
func withTLSSecret(secretName string) domainMappingOption {
	return func(r *servingv1.DomainMapping) {
		r.Spec.TLS = &servingv1.SecretTLS{
			SecretName: secretName,
		}
	}
}

func withCertificateNotRequired(dm *servingv1.DomainMapping) {
	dm.Status.MarkCertificateNotRequired(servingv1.TLSCertificateProvidedExternally)
}

func withInitDomainMappingConditions(dm *servingv1.DomainMapping) {
	dm.Status.InitializeConditions()
}

func withTLSNotEnabled(dm *servingv1.DomainMapping) {
	dm.Status.MarkTLSNotEnabled(servingv1.ExternalDomainTLSNotEnabledMessage)
}

func withCertificateNotReady(dm *servingv1.DomainMapping) {
	dm.Status.MarkCertificateNotReady(dm.Name)
}

func withHTTPDowngraded(dm *servingv1.DomainMapping) {
	dm.Status.MarkHTTPDowngrade(dm.Name)
}

func withCertificateReady(dm *servingv1.DomainMapping) {
	dm.Status.MarkCertificateReady(dm.Name)
}

func withCertificateFail(dm *servingv1.DomainMapping) {
	dm.Status.MarkCertificateProvisionFailed(dm.Name)
}

func withCertificateNotOwned(dm *servingv1.DomainMapping) {
	dm.Status.MarkCertificateNotOwned(dm.Name)
}

func withDomainClaimNotOwned(dm *servingv1.DomainMapping) {
	dm.Status.MarkDomainClaimNotOwned()
}

func withDomainClaimed(dm *servingv1.DomainMapping) {
	dm.Status.MarkDomainClaimed()
}

func withReferenceResolved(dm *servingv1.DomainMapping) {
...
func withObservedGeneration(dm *servingv1.DomainMapping) {
	dm.Status.ObservedGeneration = dm.Generation
}

func withFinalizer(dm *servingv1.DomainMapping) {
...
func ingress(dm *servingv1.DomainMapping, ingressClass string, opt ...IngressOption) *netv1alpha1.Ingress {
	return ingressWithChallenges(dm, ingressClass, nil /* challenges */, opt...)
}

func ingressWithChallenges(dm *servingv1.DomainMapping, ingressClass string, challenges []netv1alpha1.HTTP01Challenge, opt ...IngressOption) *netv1alpha1.Ingress {

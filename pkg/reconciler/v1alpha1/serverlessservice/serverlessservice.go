...
func (r *reconciler) reconcilePublicService(ctx context.Context, sks *netv1alpha1.ServerlessService) error {
	...
	if errors.IsNotFound(err) {
		...
		if err != nil {
			logger.Errorw(fmt.Sprintf("Error creating K8s Service %q", sn), zap.Error(err))
			return err
		}
		logger.Infof("Created K8s service %q", sn)
	} else if err != nil {
		logger.Errorw(fmt.Sprintf("Error getting K8s Service %q", sn), zap.Error(err))
		return err
		...
		if !equality.Semantic.DeepEqual(want.Spec, srv.Spec) {
			logger.Infof("Public K8s Service changed; reconciling %q", sn)
			if _, err = r.KubeClientSet.CoreV1().Services(sks.Namespace).Update(want); err != nil {
				logger.Errorw(fmt.Sprintf("Error updating public K8s Service %q", sn), zap.Error(err))
				return err
			}
		}
	}
	sks.Status.ServiceName = sn
	logger.Debugf("Done reconciling public K8s service %q", sn)
	return nil
}
...
func (r *reconciler) reconcilePublicEndpoints(ctx context.Context, sks *netv1alpha1.ServerlessService) error {
	...
	if err != nil {
		logger.Error(fmt.Sprintf("Error obtaining private service endpoints: %q", psn), zap.Error(err))
		return err
	}
	...
	if errors.IsNotFound(err) {
		...
		if err != nil {
			logger.Errorw(fmt.Sprintf("Error creating K8s Endpoints %q", sn), zap.Error(err))
			return err
		}
		logger.Infof("Created K8s Endpoints: %q", sn)
	} else if err != nil {
		logger.Errorw(fmt.Sprintf("Error getting K8s Endpoints %q", sn), zap.Error(err))
		return err
		...
		if !equality.Semantic.DeepEqual(want.Subsets, eps.Subsets) {
			logger.Infof("Public K8s Endpoints changed; reconciling %q", sn)
			if _, err = r.KubeClientSet.CoreV1().Endpoints(sks.Namespace).Update(want); err != nil {
				logger.Errorw(fmt.Sprintf("Error updating public K8s Endpoints %q", sn), zap.Error(err))
				return err
			}
		}
	}
	if r := presources.ReadyAddressCount(eps); r > 0 {
		logger.Infof("Endpoints %q has %d ready endpoints", sn, r)
		sks.Status.MarkEndpointsReady()
	} else {
		logger.Infof("Endpoints %q has no ready endpoints", sn)
		sks.Status.MarkEndpointsNotReady("NoHealthyBackends")
	}
	logger.Debugf("Done reconciling public K8s endpoints %q", sn)
	return nil
}
...
func (r *reconciler) reconcilePrivateService(ctx context.Context, sks *netv1alpha1.ServerlessService) error {
	...
	if !equality.Semantic.DeepEqual(svc.Spec, want.Spec) {
		...
		logger.Infof("Private K8s Service changed; reconciling %q", sn)
		if _, err = r.KubeClientSet.CoreV1().Services(sks.Namespace).Update(want); err != nil {
			logger.Errorw(fmt.Sprintf("Error updating private K8s Service %q: ", sn), zap.Error(err))
			return err
		}
	}
	sks.Status.PrivateServiceName = sn
	logger.Debugf("Done reconciling private K8s service %q", sn)
	return nil
}

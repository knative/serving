...
func validateKnativeAnnotations(annotations map[string]string) (errs *apis.FieldError) {
	annKeys := sets.StringKeySet(annotations)
	for k := range annKeys.Difference(allowedAnnotations) {
		if strings.HasPrefix(k, GroupNamePrefix) {
			errs = errs.Also(apis.ErrInvalidKeyName(k, apis.CurrentField))
		}
	}
	return
}

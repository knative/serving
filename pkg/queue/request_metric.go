...
func NewAppRequestMetricsHandler(mp metric.MeterProvider, next http.Handler, b *Breaker) (http.Handler, error) {
	var (
		meter   = mp.Meter("knative.dev/serving/pkg/queue")
		handler = &appRequestMetricsHandler{next: next, breaker: b}
		err     error
	)

...
func SuperpodProxyHandler(resourceBreaker *ResourceBreaker, stats *netstats.RequestStats, tracingEnabled bool, next http.Handler) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		stats.HandleEvent(netstats.ReqEvent{Time: time.Now(), Type: in, MemoryRequest: memReq})
		defer func() {
			stats.HandleEvent(netstats.ReqEvent{Time: time.Now(), Type: out, MemoryRequest: memReq})
		}()
		netheader.RewriteHostOut(r)

		// Enforce queuing and concurrency limits.
		if resourceBreaker != nil {
			var waitSpan *trace.Span
			if tracingEnabled {
				_, waitSpan = trace.StartSpan(r.Context(), "queue_wait")
			}
			if err := resourceBreaker.Maybe(r.Context(), uint64(memReq), func() {
				waitSpan.End()
				next.ServeHTTP(w, r)
			}); err != nil {
				waitSpan.End()

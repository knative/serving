...
INSTALL_ISTIO_YAML=""
# Used for the "Gloo Version" of Knative Serving
INSTALL_GLOO_YAML=""
...
# Installs Knative Serving and Gloo (sans Istio) in the current cluster, and waits for it to be ready.
# Parameters: $1 - Gloo YAML file
#             $2 - Knative Serving YAML file
function install_knative_serving_gloo_version() {
  INSTALL_GLOO_YAML=$1
  INSTALL_RELEASE_YAML=$2
  if [[ -z "${INSTALL_GLOO_YAML}" ]]; then
    build_knative_from_source
    INSTALL_GLOO_YAML="${GLOO_YAML}"
    INSTALL_RELEASE_YAML="${SERVING_NO_ISTIO_YAML}"
  fi

  echo ">> Installing Knative serving with Gloo"
  ...
  wait_until_service_has_external_ip gloo-system cluster-ingressgateway
}

# Uninstalls the Istio-Free Knative Serving from the current cluster.
function uninstall_knative_serving_gloo_version() {

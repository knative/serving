...
func waitForExpectedResponse(t *testing.T, clients *Clients, domain, expectedResponse string) error {
	client, err := pkgTest.NewSpoofingClient(clients.KubeClient, t.Logf, domain, ServingFlags.ResolvableDomain)
	if err != nil {
		return err
	}
	...
	_, err = client.Poll(req, RetryingRouteInconsistency(pkgTest.MatchesAllOf(pkgTest.IsStatusOK, pkgTest.EventuallyMatchesBody(expectedResponse))))
	return err
}

func ValidateDomains(
	t *testing.T, clients *Clients, baseDomain string,
...
func CheckDistribution(t *testing.T, clients *Clients, domain string, num, min int, expectedResponses []string) error {
	client, err := pkgTest.NewSpoofingClient(clients.KubeClient, t.Logf, domain, ServingFlags.ResolvableDomain)
	if err != nil {
		return err
	}
...
// RuntimeInfo object.
func FetchRuntimeInfoUnprivileged(t *testing.T, clients *Clients, options *Options, opts ...reconciler.ServiceOption) (*ResourceNames, *types.RuntimeInfo, error) {
	return runtimeInfo(t, clients, &ResourceNames{Image: runtimeUnprivileged}, options, opts...)
}
...
func FetchRuntimeInfo(t *testing.T, clients *Clients, options *Options, opts ...reconciler.ServiceOption) (*ResourceNames, *types.RuntimeInfo, error) {
	return runtimeInfo(t, clients, &ResourceNames{}, options, opts...)
}

func runtimeInfo(t *testing.T, clients *Clients, names *ResourceNames, options *Options, opts ...reconciler.ServiceOption) (*ResourceNames, *types.RuntimeInfo, error) {
	names.Service = ObjectNameForTest(t)
	if names.Image == "" {
		names.Image = runtime
	...
	CleanupOnInterrupt(func() { TearDown(clients, *names) })

	objects, err := CreateRunLatestServiceReady(t, clients, names, options, opts...)
	if err != nil {
		return nil, nil, err
	}
	...
	resp, err := pkgTest.WaitForEndpointState(
		...
		RetryingRouteInconsistency(pkgTest.IsStatusOK),
		"RuntimeInfo",
...
func ValidateRunLatestDataPlane(t *testing.T, clients *Clients, names ResourceNames, expectedText string) error {
	t.Logf("Checking that the endpoint vends the expected text: %s", expectedText)
	_, err := pkgTest.WaitForEndpointState(
		clients.KubeClient,
		t.Logf,
		names.Domain,
		RetryingRouteInconsistency(pkgTest.MatchesAllOf(pkgTest.IsStatusOK, pkgTest.EventuallyMatchesBody(expectedText))),
	...
	err = CheckServiceState(clients.ServingClient, names.Service, TODO_ServiceTrafficToRevisionWithInClusterDNS)
	if err != nil {
...
// runLatest Service's lifecycle so long as the service is in a "Ready" state.
func ValidateRunLatestControlPlane(t *testing.T, clients *Clients, names ResourceNames, expectedGeneration string) error {
	t.Log("Checking to ensure Revision is in desired state with generation: ", expectedGeneration)
	err := CheckRevisionState(clients.ServingClient, names.Revision, func(r *v1alpha1.Revision) (bool, error) {
		if ready, err := IsRevisionReady(r); !ready {
	...
	err = CheckRevisionState(clients.ServingClient, names.Revision, IsRevisionAtExpectedGeneration(expectedGeneration))
	if err != nil {
	...
	err = CheckConfigurationState(clients.ServingClient, names.Config, func(c *v1alpha1.Configuration) (bool, error) {
	...
	err = CheckRouteState(clients.ServingClient, names.Route, AllRouteTrafficAtRevision(names))
	if err != nil {
...
func ValidateLabelsPropagation(t *testing.T, objects ResourceObjects, names ResourceNames) error {
...
func ValidateAnnotations(objs *ResourceObjects) error {
...
func ValidateReleaseServiceShape(objs *ResourceObjects) error {

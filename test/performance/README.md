...
Zipkin tracing can be enabled if needed by the performance test during setup. Once it is setup, all requests made
by the `SpoofingClient` will log JSON traces for responses with HTTP server errors (response codes between 500 and 600).

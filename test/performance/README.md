...
run locally, it will create a file called `junit_knative.xml` and save it
locally under `knative/serving/test/performance/artifacts/`(Note that this
expects a directory called artifacts. Else, will fail with no directory named
artifacts error). When the test is run with Prow, this file will be stored with
the other artifacts generated by Prow like the build log.

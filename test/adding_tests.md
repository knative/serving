...
We are using the common [e2e logging library](/pkg/test/logging) that uses the 
[Knative logging library](/pkg/logging) for structured logging. It is built on top of 
[zap](https://github.com/uber-go/zap). Tests should initialize the global logger to 
use a test specifc context with `logging.GetContextLogger`:

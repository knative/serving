...
const (
	...
	// This test uses the two test.Pizza planet test images for the blue and green deployment.
...
func TestBlueGreenRoute(t *testing.T) {
	...
	g.Go(func() error {
		...
		return test.CheckDistribution(t, clients, tealDomain, test.ConcurrentRequests, min, []string{expectedBlue, expectedGreen})
	})
	g.Go(func() error {
		min := int(math.Floor(test.ConcurrentRequests * test.MinDirectPercentage))
		return test.CheckDistribution(t, clients, blueDomain, test.ConcurrentRequests, min, []string{expectedBlue})
	})
	g.Go(func() error {
		min := int(math.Floor(test.ConcurrentRequests * test.MinDirectPercentage))
		return test.CheckDistribution(t, clients, greenDomain, test.ConcurrentRequests, min, []string{expectedGreen})
	})

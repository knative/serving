...
function build_tests() {
  ...
  local result=0
  go build ./cmd/... ./sample/... ./pkg/... || result=1
  # `go build` on //test will fail with "no non-test Go files in ..."
  # See https://github.com/golang/go/issues/8279 for details
  # Workaround is to ignore that error
  local e2e_build=$(mktemp)
  go build -tags=e2e ./test/... 2>&1 | tee ${e2e_build}
  if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
    result=1
    # If build failed, assume pass if we only got the "non-test Go files" error
    if [[ -z "$(grep -v ': no non-test Go files' ${e2e_build})" ]]; then
      result=0
    fi
  fi

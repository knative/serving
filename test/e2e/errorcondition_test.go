...
func TestContainerErrorMsg(t *testing.T) {
	...
	err = test.WaitForConfigurationState(h.Clients.Configs, h.ConfigName, func(r *v1alpha1.Configuration) (bool, error) {
		...
		if cond != nil {
			...
			s := fmt.Sprintf("The configuration %s was not marked with expected error condition with Reason to be \"%s\", Message to be \"%s\", and Status to be \"%s\"", h.ConfigName, cond.Reason, cond.Message, cond.Status)
	...
	if err != nil {
		t.Fatalf("Failed to get configuration state from configuration %s: %v", h.ConfigName, err)
	}

	revisionName, err := h.getRevisionFromConfiguration()
	if err != nil {
		t.Fatalf("Failed to get revision from configuration %s: %v", h.ConfigName, err)
	}

	log.Println("When the imagepath is invalid, the revision should have error status.")
	err = test.WaitForRevisionState(h.Clients.Revisions, revisionName, func(r *v1alpha1.Revision) (bool, error) {
	...
	logURL, err := h.getLogURLFromRevision(revisionName)
	if err != nil {
...
func (h *Helper) getRevisionFromConfiguration() (string, error) {
	...
	s := fmt.Sprintf("No valid revision name found in configuration %s", h.ConfigName)
	return "", errors.New(s)
}

// Get LogURL from revision.
func (h *Helper) getLogURLFromRevision(revisionName string) (string, error) {
	revision, err := h.Clients.Revisions.Get(revisionName, metav1.GetOptions{})
	if err != nil {
		return "", err
	}

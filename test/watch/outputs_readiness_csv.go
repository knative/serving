...
func (o *CSVWriter) WriteHistory(h GVRHistory) {
	...
	for gvr, itemHistory := range h {
		...
		for name, history := range itemHistory {
			...
			readyTimeString, obj, ok := getReadyTime(history)
			if !ok {
...
func getReadyTime(history *ObjectHistory) (string, *unstructured.Unstructured, bool) {
	for _, event := range history.WatchEvents {
		if strings.EqualFold(event.Object.GetKind(), "replicaset") {
			return getReplicaReadyTime(history)
		}
...
func getReplicaReadyTime(history *ObjectHistory) (string, *unstructured.Unstructured, bool) {
	obj := history.WatchEvents[0].Object
	for _, event := range history.WatchEvents {
		replicas, _, _ := unstructured.NestedString(event.Object.Object, "spec", "replicas")
		readyReplicas, _, _ := unstructured.NestedString(event.Object.Object, "status", "readyReplicas")
		if replicas == readyReplicas {
			return event.ReceiveTime.Format(time.RFC3339), obj, true
		}
	}
	return "", nil, false
}

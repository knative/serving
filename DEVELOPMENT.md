...
```shell
kubectl apply -f ./third_party/istio-1.4-latest/istio-crds.yaml
while [[ $(kubectl get crd gateways.networking.istio.io -o jsonpath='{.status.conditions[?(@.type=="Established")].status}') != 'True' ]]; do
  echo "Waiting on Istio CRDs"; sleep 1
done
kubectl apply -f ./third_party/istio-1.4-latest/istio-minimal.yaml
```
...
```shell
kubectl apply -f ./third_party/istio-1.4-latest/istio-knative-extras.yaml
```
...
ko delete --ignore-not-found=true \
  ...
  -f config/ \
  -f ./third_party/istio-1.4-latest/istio-minimal.yaml \
  -f ./third_party/istio-1.4-latest/istio-crds.yaml \

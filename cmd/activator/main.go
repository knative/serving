...
func main() {
	...
	// Enable TLS for connections to queue-proxy when knative-internal-tls is enabled.
	tlsEnabled := networkConfig.KnativeInternalTLSEnabled()
	...
	// Enable TLS server when knative-internal-tls is specified.
	// At this moment activator with TLS does not disable HTTP.
	// See also https://github.com/knative/serving/issues/12808.
	if tlsEnabled {
